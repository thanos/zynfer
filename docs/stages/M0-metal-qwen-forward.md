...
**Status: in progress (baseline landed).** First Qwen3 forward and
KV-cached generate on Metal, f32, correctness before speed.
...
1. Greedy tokens / logits match CPU golden on fixture prompts.
2. Per-layer tolerances documented (f32, 3e-3 abs typical on mini).
